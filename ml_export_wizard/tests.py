...
class UtilsTests(TestCase):
    ...
    def test_parse_formula_to_sql_should_raise_exception_given_invalid_enclosed_field_name(
        self,
    ):
        ...
        with self.assertRaises(MLExportWizardFieldNotFound):
            sql = exporters["Integrations"]._resolve_formula(
                formula='test("bad_field")'
            )

    def test_parse_formula_to_sql_should_return_formula_with_fields_given_valid_enclosed_field_name(
        self,
    ):
    ...
    def test_parse_formula_to_sql_should_return_formula_with_fields_given_valid_enclosed_field_name_at_query_layer(
        self,
    ):
        """parse_formula_to_sql should return formula with fields given valid enclosed field name at query layer"""
        sql = exporters["Integrations"]._resolve_formula(
            formula='test("ltr")', query_layer="base"
        )
        self.assertEqual(sql, "test(integrations.ltr)")

        sql = exporters["Integrations"]._resolve_formula(
            formula='test("ltr")', query_layer="middle"
        )
        self.assertEqual(sql, "test(integrations_ltr)")

    def test_parse_formula_to_sql_should_raise_exception_givein_invalid_query_layer(
        self,
    ):
        """parse_formula_to_sql should raise exception given invalid query layer"""
        with self.assertRaises(ValueError):
            sql = exporters["Integrations"]._resolve_formula(
                formula='test("ltr")', query_layer="bad"
            )
    ...
    def test_merge_sql_dicts_should_return_items_from_one_dict_when_the_other_is_empty(
        self,
    ):
