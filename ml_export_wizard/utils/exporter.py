...
from ml_export_wizard.exceptions import (
    MLExportWizardFieldNotFound,
    MLExportWizardQueryExecuting,
    MLExportWizardQueryNotExecuted,
    MLExportWizardExporterNotFound,
)
...
class BaseExporter(object):
    ...
    def __init__(self, parent: object = None, name: str = "", **settings) -> None:
        ...
        if parent:
            self.parent = proxy(
                parent
            )  # Use a weakref so we don't have a circular refrence, defeating garbage collection
        self.name = name
        self.settings = {}
...
class Exporter(BaseExporter):
    ...
    def xlsx(
        self, *, query: "ExporterQuery", file_name: str = None, title: str = None
    ) -> str | None:
    ...
    def base_sql(
        self, *, sql_only: bool = False, query: "ExporterQuery", query_layer: str = None
    ) -> tuple[str, dict] | str:
        ...
        for app in self.apps:
            sql_dict = merge_sql_dicts(
                sql_dict, app._sql_dict(query=query, query_layer=query_layer)
            )

        for rollup in self.rollups:
            sql_dict = merge_sql_dicts(
                sql_dict, rollup._sql_dict(query=query, query_layer=rollup_query_layer)
            )

        # Add the extra fields
        for column in self.settings.get("extra_field", []):
            added_select_bit, added_parameters = self._resolve_extra_field(
                column=column,
                query_layer=query_layer,
                field_query_layer=field_query_layer,
            )

            sql_dict["select"] = ", ".join(
                filter(None, (sql_dict.get("select"), added_select_bit))
            )
        ...
        if sql_only:
            return sql % {
                key: f"'{value}'"
                for (key, value) in sql_dict.get("parameters", {}).items()
            }
    ...
    def query_count(self, *, query: "ExporterQuery") -> int:
        ...
        if query.count and query.count.startswith("DISTINCT:"):
            ...
            query.extra_field.append({"function": "count", "source_field": query.count})
    ...
    def find_field(self, app: str = None, model: str = None, field: str = None):
        ...
        if app and model and field:
            try:
                ...
                raise MLExportWizardFieldNotFound(
                    f"Field not found in exporter (exporter: {self.name}, app: {app}, model: {model}, field: {field})"
                )
            ...
            if len(fields) == 0:
                raise MLExportWizardFieldNotFound(
                    f"Field not found in exporter (exporter: {self.name}, field: {field})"
                )
            elif len(fields) == 1:
                field_object = fields[0]
            else:
                raise MLExportWizardFieldNotFound(
                    f"Field is ambiguous, try indicating the model or app (exporter: {self.name}, field: {field})"
                )

        else:
            raise MLExportWizardFieldNotFound(
                f"Lookup for undefined field failed (exporter: {self.name})"
            )
        ...
        *,
        ...
        if query.group_by:
            for group in query.group_by:
                ...
                sql_dict["group_by"] = ", ".join(
                    filter(
                        None,
                        (
                            sql_dict.get("group_by"),
                            field.column(query_layer=field_query_layer),
                        ),
                    )
                )
                sql_dict["select"] = ", ".join(
            ...
            for order_set in query.order_by:
                ...
                if type(order_field) is str:
                    ...
                    if "formula" in order_field:
                        field = self._resolve_formula(
                            formula=order_field["formula"],
                            query_layer=field_query_layer,
                        )
                        explicit_field = True
                    else:
                        field = self.find_field(
                            app=order_field.get("app"),
                            model=order_field.get("model"),
                            field=order_field.get("field"),
                        )
                ...
                sql_dict["order_by"] = ", ".join(
                    filter(None, (sql_dict.get("order_by"), order_bit))
                )
        ...
        if query.where:
            for where_item in query.where:
                ...
                if type(where_item["field"]) is str:
                    ...
                    field = self.find_field(
                        app=where_item["field"].get("app"),
                        model=where_item["field"].get("model"),
                        field=where_item["field"].get("field"),
                    )
                ...
                if "value" in where_item:
                    ...
                    where_bit = (
                        f"{field.column(query_layer=field_query_layer)} IS NOT NULL"
                    )
                ...
                sql_dict["where"] = " AND ".join(
                    filter(None, (sql_dict.get("where"), where_bit))
                )
                parameters = parameters | added_parameters
    ...
    def _resolve_extra_field(
        self,
        *,
        column: dict = None,
        query_layer: str = None,
        field_query_layer: str = None,
    ) -> tuple[str, dict]:
        ...
        if source_field := column.get("source_field"):
            field_column = self._resolve_source_field(
                source_field=source_field, field_query_layer=field_query_layer
            )

        elif case_expression := column.get("case_expression"):
            field_column = self._resolve_source_field(
                source_field=case_expression.get("source_field"),
                field_query_layer=field_query_layer,
                operator=case_expression.get("operator"),
            )

        if column.get("formula"):
            select_bit = self._resolve_formula(
                formula=column["formula"], query_layer=field_query_layer
            )
        ...
        match column.get("function"):
            ...
            case "aggragate":
                if field_column:
                    ...
                    if column.get("order"):
                        order_bit = (
                            f" ORDER BY {field_column}{field_cast} {column['order']}"
                        )
                    ...
                    if when.get("condition"):
                        ...
                        if when.get("extra_field"):
                            (
                                added_select_bit,
                                added_parameters,
                            ) = self._resolve_extra_field(
                                column=when["extra_field"],
                                field_query_layer=field_query_layer,
                            )
                        elif when.get("value"):
                            added_parameters[
                                f"{column['column_name']}_{column['when'].index(when)}_value"
                            ] = when["value"]
                        ...
                        parameters[
                            f"{column['column_name']}_{column['when'].index(when)}"
                        ] = when["condition"]
                        parameters = parameters | added_parameters
        ...
        if column.get("filter"):
            ...
            for filter_item in column.get("filter", []):
                ...
                where_bit, added_parameters = _resolve_where(
    ...
    def _resolve_source_field(
        self,
        *,
        source_field: str | dict | list = None,
        field_query_layer: str = None,
        operator: str = None,
    ) -> str:
        ...
        for source_field in source_fields:
            if type(source_field) is str:
                ...
                fields.append(
                    self.find_field(
                        app=source_field.get("app"),
                        model=source_field.get("model"),
        ...
        if len(fields) == 1:
            ...
            if operator == "join":
                return " || ".join(
                    [field.column(query_layer=field_query_layer) for field in fields]
                )
    ...
    def _resolve_formula(self, *, formula: str, query_layer: str = None) -> str:
        ...
        if query_layer and query_layer not in ["base", "middle"]:
            raise ValueError(
                "Can only use query_layer 'base' or 'middle' when resolving formula"
            )

        for field in re.finditer('".*?"', formula):
            field_name = self.find_field(field=field.group().replace('"', "")).column(
                query_layer=query_layer
            )
...
class ExporterQuery(object):
    ...
    def __init__(
        ...
        *,
        ...
        count: str = None,
        group_by: dict | list | str = None,
        where_before_join: dict = None,
        where: dict = None,
        extra_field: dict | list = None,
    ...
    def is_update_safe(self) -> bool:
        ...
        if self._is_executing:
            raise MLExportWizardQueryExecuting(
                "Cannot update query while it is executing"
            )
    ...
    def rowcount(self) -> int:
        ...
        if self._cursor is None:
            raise MLExportWizardQueryNotExecuted(
                "Cannot get row count before query is executed"
            )
    ...
    def columns(self) -> list:
        ...
        if self._cursor is None:
            raise MLExportWizardQueryNotExecuted(
                "Cannot get columns before query is executed"
            )
...
class ExporterApp(BaseExporter):
    ...
    def _sql_dict(
        self, *, query: "ExporterQuery", query_layer: str = None
    ) -> dict[str:str]:
        ...
        if "primary_model" not in self.settings or not self.settings["primary_model"]:
            raise ImproperlyConfigured(
                f"No valid primary_model in Exporter: {self.parent.name}, App: {self.name}"
            )

        sql_dict = self.models_by_name[self.settings["primary_model"]]._sql_dict(
            query=query, query_layer=query_layer
        )
...
class ExporterModel(BaseExporter):
    ...
    def __init__(
        self, *, parent: object = None, name: str = "", model: object, **settings
    ) -> None:
        """Initialize the object"""
    ...
    def _sql_dict(
        ...
        *,
        ...
        if query.where_before_join and self.name in query.where_before_join:
            ...
            for field_name, value, operator in [
                (limit.get("field"), limit.get("value"), limit.get("operator", "="))
                for limit in query.where_before_join[self.name]
            ]:
                field = self.fields_by_name[field_name]

                where_bit, parameters = _resolve_where(
        ...
        if join_model:
            join_criteria = (
                f"ON {self.table}.{join_using} = {join_model.table}.{join_using}"
            )
        else:
        ...
        for field in self.fields:
            sql_dict = merge_sql_dicts(
                sql_dict,
                field._sql_dict(
                    query=query, table_name=table_name, query_layer=query_layer
                ),
            )
        ...
        for field in self.fields:
            ...
            if (
                type(field_object)
                in (models.ManyToOneRel, models.ForeignKey, models.OneToOneRel)
                and field_object.related_model.__name__ in self.parent.models_by_name
            ):
                join_model = self.parent.models_by_name[
                    field_object.related_model.__name__
                ]
                ...
                if join_model in self.parent.prepped_models or (
                    "dont_link_to" in self.settings
                    and join_model.name in self.settings["dont_link_to"]
                    or "dont_link_to" in join_model.settings
                    and self.name in join_model.settings["dont_link_to"]
                ):
                    continue
                ...
                sql_dict = merge_sql_dicts(
                    sql_dict,
                    join_model._sql_dict(
                        left_join=True,
                        join_model=self,
                        join_using=field.column(),
                        query=query,
                        query_layer=query_layer,
                    ),
                )
...
class ExporterRollup(BaseExporter):
    ...
    def __init__(
        self, *, parent: object = None, name: str = "", exporter: str = None, **settings
    ) -> None:
        """Initialise the object"""
        ...
        for field in self.settings.get("group_by", []):
            if "." in field:
                app, model, field = field.split(".")
                field_object = self.exporter.find_field(
                    app=app, model=model, field=field
                )
            else:
        ...
        for extra_field in self.settings.get("extra_field", []):
            pseudofield = ExporterPseudofield(
                parent=self, name=extra_field["column_name"]
            )
    ...
    def _sql_dict(
        ...
        from_bit, sql_dict["parameters"] = self.exporter.final_sql(
            query=rollup_query, query_layer="inner"
        )
        ...
        for field in self.fields:
            sql_dict = merge_sql_dicts(
                sql_dict,
                field._sql_dict(
                    query=rollup_query, table_name=self.name, query_layer=query_layer
                ),
            )

        for extra_field in self.settings.get("extra_field", []):
            sql_dict["select"] = ", ".join(
                filter(None, (sql_dict["select"], extra_field["column_name"]))
            )
...
class ExporterField(BaseExporter):
    ...
    def __init__(
        self, *, parent: object = None, name: str = "", field: object = None, **settings
    ) -> None:
        """Initialise the object"""
    ...
    def _sql_dict(
        self, *, query: ExporterQuery, table_name: str = None, query_layer: str = None
    ) -> dict[str:str]:
        ...
        if self.settings.get("join_only") or type(self.field) in (
            models.ManyToOneRel,
...
def _resolve_where(
    *,
    operator: str = None,
    field: ExporterField | ExporterPseudofield = None,
    ...
    if operator in ("=", ">=", "<=", "<>", "!="):
        where_bit = "AND ".join(
            filter(None, (where_bit, f"{field_column}{operator}%({field_column})s"))
        )
...
def setup_exporters() -> None:
    ...
    for exporter in settings.ML_EXPORT_WIZARD["Exporters"]:
        ...
        exporter_settings = {
            setting: value
            for setting, value in exporter.items()
            if setting not in exclude_keys
        }
        working_exporter = exporters[exporter["name"]] = Exporter(
            name=exporter["name"],
            long_name=exporter.get("long_name", ""),
            description=exporter.get("description", ""),
            **exporter_settings,
        )
        ...
        for app in exporter.get("apps", []):
            ...
            app_settings = {
                setting: value
                for setting, value in app.items()
                if setting not in exclude_keys
            }
            working_app: ExporterApp = ExporterApp(
                parent=working_exporter, name=app.get("name", ""), **app_settings
            )
            ...
            if "include_models" in app:
                models = [
                    apps.get_model(app_label=app["name"], model_name=model)
                    for model in app["include_models"]
                ]
            ...
            if not models:
                models = [
                    model
                    for model in apps.get_app_config(app["name"]).get_models()
                    if model.__name__ not in app.get("exclude_models", [])
                ]
            ...
            for model in models:
                ...
                for field in [
                    field
                    for field in working_model.model._meta.get_fields()
                    if field.name
                    not in working_exporter.settings.get("exclude_fields", [])
                ]:
                    working_field = ExporterField(
                        parent=working_model, name=field.name, field=field
                    )
        ...
        for rollup in exporter.get("rollups", []):
            ...
            rollup_settings = {
                setting: value
                for setting, value in rollup.items()
                if setting not in exclude_keys
            }
            working_rollup: ExporterRollup = ExporterRollup(
                parent=working_exporter,
                name=rollup.get("name"),
                exporter=rollup.get("exporter"),
                **rollup_settings,
            )
