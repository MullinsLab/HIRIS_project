...
class ModelTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.my_genome_species = GenomeSpecies.objects.create(
            genome_species_name="My Genome Species"
        )
        cls.my_genome_version = GenomeVersion.objects.create(
            genome_version_name="My Genome", genome_species=cls.my_genome_species
        )
        ...
        cls.my_feature = Feature.objects.create(
            feature_name="My Gene",
            genome_version=cls.my_genome_version,
            feature_type=cls.my_feature_type,
            gene_type=cls.my_gene_type,
        )
        cls.my_feature_location = FeatureLocation(
            feature=cls.my_feature,
            chromosome="chr1",
            landmark="NC_00002.2",
            feature_start=1231,
            feature_end=234234,
            feature_orientation="F",
        )

    def test_core_base_class_should_return_names_that_match_objects_name_attribute(
        self,
    ):
    ...
    def test_core_base_class_should_return_correct_str_for_objects_with_no_name_attribute(
        self,
    ):
