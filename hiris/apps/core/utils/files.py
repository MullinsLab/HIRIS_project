...
def integrations_bed(*, environment: str, title: str=None, file_name: str=None) -> str|None:
    ...
    for row in query.get_dict_list_generator():
        if chromosome := db.get_chromosome_from_landmark(landmark=row["landmark"], genome_version=row["genome_version_name"]):
...
def integration_gene_summary_gff3(*, gene: str=None, file_name: str=None) -> str|None:
    ...
    if gene:
        query: ExporterQuery = exporters["IntegrationsGeneSummary"].query(where=[{"field": "feature_name", "value": gene}])
    else:
        query: ExporterQuery = exporters["IntegrationsGeneSummary"].query(where=[{"field": "feature_name", "valid": True}])
    ...
    for row in query.get_dict_list_generator():
        orientation: str = "+" if row["orientation_in_landmark"] == "F" else ("-" if row["orientation_in_landmark"] == "R" else ".")
        location: int = row["location"]-row["feature_start"]+1 # Add 1 to the start position to make it 1-based
        ...
        attributes += f";Name={row['subject_identifier'] or row['integration_environment_name']}"
