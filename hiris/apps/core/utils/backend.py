...
def fill_publication_data() -> None:
    ...
    publications: list[Publication] = Publication.objects.filter(
        publication_data=None, publication_pubmed_id__isnull=False
    )
    data: dict = get_pubmed_data(
        pubmed_id=[publication.publication_pubmed_id for publication in publications]
    )
                ...
                PublicationData.objects.bulk_create(
                    [
                        PublicationData(
                            ...
                            key="first_author",
                            value=publication_data["authors"][0]["name"],
                        ),
                        PublicationData(
                            publication=publication,
                            key="title",
                            value=publication_data["title"],
                        ),
                    ]
                )
