...
from ml_import_wizard.models import (
    ImportScheme,
    ImportSchemeFile,
    ImportSchemeItem,
    ImportSchemeFileField,
)
from ml_import_wizard.utils.simple import (
    sound_user_name,
    resolve_true,
    table_resolve_key_values_to_string,
)
...
class ManageImports(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for importer, importer_dict in settings.ML_IMPORT_WIZARD["Importers"].items():
            ...
            if description := importer_dict.get("description"):
                importer_item["description"] = description
        ...
        return render(
            request,
            "ml_import_wizard/manager.html",
            context={
                "importers": importers,
                "user_import_schemes": user_import_schemes,
            },
        )
...
class NewImportScheme(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "ml_import_wizard/new_scheme.html",
            context={
                "form": NewImportSchemeForm(
                    importer_slug=importer,
                    initial={
                        "name": f"{sound_user_name(request.user)}'s {importer_name} import"
                    },
                ),
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse(
                    "ml_import_wizard:scheme",
                    kwargs={"import_scheme_id": import_scheme.id},
                )
            )
...
class DoImportScheme(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        """Do the actual import stuff"""
        import_scheme_id: int = kwargs.get(
            "import_scheme_id", request.session.get("current_import_scheme_id")
        )
        ...
        return render(
            request,
            "ml_import_wizard/scheme.html",
            context={
                "importer": settings.ML_IMPORT_WIZARD["Importers"][
                    import_scheme.importer
                ]["name"],
...
class ListImportSchemeItems(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        import_scheme: ImportScheme = ImportScheme.objects.get(
            pk=kwargs["import_scheme_id"]
        )
        ...
        if (
            not import_scheme.files.count()
            or not file_settings["inspected"]
            or (
                import_scheme.files.count() >= 2
                and (
                    not import_scheme.settings.get("primary_file_id", None)
        ...
        return JsonResponse({"import_scheme_items": import_scheme_items})
            ...
            if import_scheme.files.count() == 0:
                return_data = {
                    ...
                    "form": render_to_string(
                        "ml_import_wizard/fragments/scheme_file.html",
                        request=request,
                        context={
                            "form": UploadFileForImportForm(),
                            "path": reverse(
                                "ml_import_wizard:scheme_item",
                                kwargs={
                                    "import_scheme_id": import_scheme.id,
                                    "import_item_id": 0,
                                },
                            ),
                        },
                ...
                name: str = (
                    ...
                    else f"{import_scheme.files.count()} files uploaded"
                )
                description: str = (
                    "There is 1 file"
                    if len(files) == 1
                    else f"There are {len(files)} files"
                ) + " uploaded for this import."
                ...
                field_list: list = (
                    []
                )  # List of fields in the form.  Used to check completeness
                ...
                needs_linking: bool = (
                    False  # files in the scheme haven't been linked yet
                )
                needs_primary: bool = (
                    False  # the scheme doesn't have a primary file yet
                )
                ...
                files_excluding_master: list[
                    ImportSchemeFile
                ] = []  # List of all the files that aren't the master
                ...
                if not field_list:
                    ...
                    if not import_scheme.settings.get("primary_file_id"):
                        if len(files) == 1:
                            ...
                            start_expanded = (
                                ...
                                needs_form
                            ) = needs_primary = hide_file_list = tooltip = True
                    ...
                    # Present form if files have not been linked togehter, but only if everything is inspected
                        ...
                        for file in files:
                            if file.id == int(
                                import_scheme.settings.get("primary_file_id")
                            ):
                                primary_file = file
                            else:
                        ...
                        start_expanded = (
                            ...
                            needs_linking
                        ) = hide_file_list = selectpicker = tooltip = True

                list_bit = render_to_string(
                    "ml_import_wizard/fragments/file_list.html",
                    request=request,
                    context={
                        "files": import_scheme.files.all(),
        ...
        if import_item_id == 0:
            ...
            if resolve_true(request.POST.get("---file_saved---", False)):
                ...
                for attribute, value in request.POST.items():
                    if attribute in ("csrfmiddlewaretoken", "---file_saved---"):
                        continue

                    if "first_row_header_" in attribute:
                        import_scheme_file = ImportSchemeFile.objects.get(
                            pk=attribute.split("_")[-1]
                        )
                    ...
                    if "linked-" in attribute:
                        ...
                        linked_files[file_id] = {
                            ...
                            "primary": int(
                                request.POST.get(f"primary-{file_id}", "").replace(
                                    "**field**", ""
                                )
                            ),
                        }
                ...
                for import_scheme_file in check_for_inspect:
                    if import_scheme_file.ready_to_inspect:
                        ...
                        os.popen(
                            os.path.join(settings.BASE_DIR, "manage.py inspect_file ")
                            + str(import_scheme_file.id)
                        )
                ...
                if form.is_valid():
                    for file in request.FILES.values():
                        import_file = ImportSchemeFile(
                            name=file.name, import_scheme=import_scheme
                        )
                        import_file.save()
                        with open(
...
class DoImporterModel(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        model_object = (
            importers[import_scheme.importer].apps_by_name[app].models_by_name[model]
        )

        field_values: dict[str:list] = {}  # Allowable values for fields
        field_list: list[
            str
        ] = []  # List of fields for the javascript to itterate through
        field_strategies: dict[
            str:any
        ] = {}  # List of field strategies to fill the form from
        show_files: bool = (
            True if import_scheme.files.count() > 1 else False
        )  # Supress file name in selector if there is only one file
        is_key_value_model: bool = (
            False  # Indicates that this model is a key_value_model
        )
        ...
        if model_object.settings.get("key_value_model"):
            ...
            keys_from_db = [
                getattr(object, key_field)
                for object in model_object.model.objects.order_by(key_field).distinct(
                    key_field
                )
            ]
            key_value_model_keys = list(
                set(
                    sorted(
                        keys_from_db + model_object.settings.get("initial_values", [])
                    )
                )
            )

            try:
                import_item: ImportSchemeItem = import_scheme.items.get(
                    app=app, model=model, field="**key_value**"
                )
            except:
                import_item = None
            ...
            if import_item:
                ...
                for setting, value in import_item.settings.items():
                    file_field: ImportSchemeFileField = (
                        ImportSchemeFileField.objects.get(pk=value.get("key"))
                    )
                    key_value_model_setup.append(
                        {
        ...
        # Stuff for standard models
            ...
            for field in model_object.settings.get("load_value_fields", []):
                field_values[field] = model_object.model.objects.values_list(
                    field, flat=True
                )
            ...
            for field in model_object.shown_fields:
                ...
                items = import_scheme.items.filter(
                    app=app, model=model, field=field.name
                )
                if items.count() == 0:
                    continue
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get("**is_key_value_model**"):
            ...
            import_scheme.create_or_update_item(
                app=app, model=model, field=field, strategy=strategy, settings=settings
            )
            ...
            for field, values in fields.items():
                ...
                if value == "**raw_text**":
                    ...
                    for file_field in [
                        f"file_field_first_{count}" for count in [1, 2, 3]
                    ]:
                        if values[file_field]:
                            settings["first_keys"].append(
                                int(values[file_field].split("**field**")[1])
                            )

                elif value == "**split_field**":
                    strategy = "Split Field"

                    settings["split_key"] = int(
                        values["file_field_split"].split("**field**")[1]
                    )
                    settings["splitter"] = values["file_field_split_splitter"]
                    settings["splitter_position"] = int(
                        values["file_field_split_position"]
                    )
                    ...
                    for argument, argument_value in [
                        (argument.split(":")[-1], argument_value)
                        for argument, argument_value in values.items()
...
class PreviewImportScheme(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "ml_import_wizard/scheme_preview.html",
...
class DescribeImportScheme(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "ml_import_wizard/scheme_description.html",
