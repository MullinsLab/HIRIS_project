...
if find_spec("gffutils"):
    import gffutils
else:
    NO_GFFUTILS = True

from ml_import_wizard.utils.simple import (
    dict_hash,
    stringalize,
    fancy_name,
    deep_exists,
)
from ml_import_wizard.exceptions import (
    GFFUtilsNotInstalledError,
    FileNotReadyError,
    ImportSchemeNotReady,
    StatusNotFound,
)
...
class ImportScheme(ImportBaseModel):
    ...
    def save(self, *args, **kwargs) -> None:
        ...
        if not self.importer_hash:
            self.importer_hash = dict_hash(
                settings.ML_IMPORT_WIZARD["Importers"][self.importer]
            )
    ...
    def create_or_update_item(
        self, *, app: str, model: str, field: str, strategy: str, settings: dict
    ) -> ImportBaseModel:
    ...
    def preview_data_table(self, limit_count: int = 100) -> dict:
        ...
        if self.status.import_defined == False:
            raise ImportSchemeNotReady(
                f"Import scheme {self.name} ({self.id}) has not been set up."
            )

        table: dict = {"columns": self.data_columns(), "rows": []}
        ...
        for app in importer.apps:
            for model in app.models:
                if model.is_key_value:
                    try:
                        import_scheme_item = ImportSchemeItem.objects.get(
                            import_scheme_id=self.id,
                            app=app.name,
                            model=model.name,
                            field="**key_value**",
                        )
    ...
    def data_rows(
        self, *, columns: list = None, limit_count: int = None, offset_count: int = 0
    ) -> dict[str:any]:
        ...
        if self.files.count() > 1:
            ...
            for file, value in self.settings["file_links"].items():
                ...
                import_scheme_file_field = ImportSchemeFileField.objects.get(
                    pk=value["child"]
                )
        ...
        for row in primary_file.rows(
            ...
            for column in [
                column
                for column in columns
                if column["import_scheme_item"].strategy not in (deferred_strategies)
            ]:
                ...
                if column["importer_model"].is_key_value:
                    ...
                    for key_value_key, key_value_value in column[
                        "import_scheme_item"
                    ].settings.items():
                        if type(key_value_value) is dict and "key" in key_value_value:
                            row_dict[column["name"]][
                                key_value_key
                            ] = self.key_to_file_field(
                                fields,
                                primary_file,
                                child_files,
                                row,
                ...
                # The 'regular' import of a field directly from the file
                    ...
                    row_dict[column["name"]] = self.key_to_file_field(
                        fields, primary_file, child_files, row, key
                    )
                    ...
                    for key in settings.get("first_keys", []):
                        if key not in fields:
                            import_scheme_file_field = (
                                ImportSchemeFileField.objects.get(pk=key)
                            )
                            fields[import_scheme_file_field.id] = {
                                "name": import_scheme_file_field.name,
                                "file": import_scheme_file_field.import_scheme_file_id,
                    ...
                    if value and settings["splitter"] in value:
                        row_dict[column["name"]] = value.split(settings["splitter"])[
                            settings["splitter_position"] - 1
                        ]
                    else:
                        row_dict[column["name"]] = value
                ...
                if column["name"] in row_dict:
                    if (
                        type(row_dict[column["name"]]) is str
                        and row_dict[column["name"]].lower() == "null"
                    ):
                        row_dict[column["name"]] = None
                    ...
                    if "translate_values" in column["importer_field"].settings:
                        if (
                            row_dict[column["name"]]
                            in column["importer_field"].settings["translate_values"]
                        ):
                            row_dict[column["name"]] = column[
                                "importer_field"
                            ].settings["translate_values"][row_dict[column["name"]]]

                    if (
                        column["importer_field"].settings.get("force_case") == "upper"
                        and row_dict[column["name"]]
                    ):
                        row_dict[column["name"]] = row_dict[column["name"]].upper()

                    if (
                        column["importer_field"].settings.get("force_case") == "lower"
                        and row_dict[column["name"]]
                    ):
                ...
                if column["importer_model"].settings.get("restriction") == "rejected":
                    if "approved_values" in column["importer_field"].settings:
                        if row_dict[column["name"]] not in column[
                            "importer_field"
                        ].settings.get("approved_values", []):
                            if "reject_row" not in row_dict["***row***setting***"]:
                                row_dict["***row***setting***"]["reject_row"] = []

                            row_dict["***row***setting***"]["reject_row"].append(
            ...
            for column in [
                column
                for column in columns
                if column["import_scheme_item"].strategy in (deferred_strategies)
            ]:
                strategy = column["import_scheme_item"].strategy
                settings = column["import_scheme_item"].settings
                ...
                if strategy == "Resolver":
                    ...
                    for argument in resolver["user_input_arguments"]:
                        ...
                        arguments[
                            f"user_input_{argument['name']}"
                        ] = self.key_to_file_field(
                            fields, primary_file, child_files, row, key
                        )

                    if not (
                        field_value := deferred_cache.find(key=dict_hash(arguments))
                    ):
                        field_value = resolver["function"](**arguments)
                        deferred_cache.store(
                            key=dict_hash(arguments), value=field_value
                        )
    ...
    def execute(
        ...
        *,
        ...
        if not ignore_status and self.status.import_completed == True:
            raise ImportSchemeNotReady(
                f"Import scheme {self.name} ({self.id}) has already been imported."
            )
        ...
        for row in self.data_rows(
            columns=columns, limit_count=limit_count, offset_count=offset_count
        ):
            # log.debug(row)
            if not offset_count:
                offset_count = 0
            ...
            if deep_exists(dictionary=row, keys=["***row***setting***", "reject_row"]):
                ImportSchemeRowRejected(
                    import_scheme=self,
                    errors=row["***row***setting***"]["reject_row"],
                    row=row,
                ).save()
                continue
            ...
            try:
                with transaction.atomic():
                    ...
                    for app in importers[self.importer].apps:
                        ...
                        for model in app.models_by_import_order:
                                ...
                                for key, value in [
                                    ...
                                    for field in model.fields:
                                        if field.is_foreign_key:
                                            if (
                                                field.field.related_model.__name__
                                                in working_objects
                                            ):
                                                working_attributes[
                                                    field.name
                            ...
                            superbreak: bool = (
                                False  # Needed to break out of both for loops
                            )
                            ...
                            unique_sets: list[tuple] = list(
                                model.model._meta.__dict__.get("unique_together")
                            )
                            unique_sets = unique_sets + [
                                (field.name,)
                                for field in model.fields
                                if field.field.unique and not field.is_foreign_key
                            ]

                            # minimum_objects models treat all fields together as unique so we don't end up with duplicates
                            if (
                                "minimum_objects" not in model.settings
                                or model.settings["minimum_objects"]
                            ):
                                full_unique_set: list = [
                            ...
                            for unique_set in unique_sets:
                                ...
                                if "***Key_Value_Models***" in unique_set:
                                    for key_value_model in model.key_value_children:
                                        key_value_attributes[key_value_model.name] = {
                                            key: value
                                            for key, value in row.get(
                                                f"{key_value_model.name} (key-value)",
                                                {},
                                            ).items()
                                            if value and value != "NULL"
                                        }
                                ...
                                for unique_field in [
                                    ...
                                    if unique_field in working_attributes
                                ]:
                                    test_attributes[
                                        getattr(unique_field, "name", unique_field)
                                    ] = working_attributes[unique_field]
                                    test_attributes_string += f"|{unique_field}:{working_attributes[unique_field]}|"

                                temp_object: any = cache_thing.find(
                                    key=(model.name, test_attributes_string),
                                    report=False,
                                )
                                ...
                                if (
                                    model.name not in working_objects
                                    or not working_objects[model.name]
                                ):
                                    temp_object = model.model.objects.filter(
                                        **test_attributes
                                    )
                                    ...
                                    if key_value_attributes:
                                        for key_value_model in model.key_value_children:
                                            temp_object = temp_object.annotate(
                                                key_value_count=Count(
                                                    key_value_model.table
                                                )
                                            ).filter(
                                                key_value_count=len(
                                                    key_value_attributes[
                                                        key_value_model.name
                                                    ]
                                                )
                                            )

                                            for key, value in key_value_attributes[
                                                key_value_model.name
                                            ].items():
                                                attributes: dict = {
                                                ...
                                                temp_object = temp_object.filter(
                                                    **attributes
                                                )
                                    ...
                                    if temp_object:
                                        ...
                                        cache_thing.store(
                                            key=(model.name, test_attributes_string),
                                            value=working_objects[model.name],
                                            transaction=True,
                                        )
                            ...
                            if model.name not in working_objects:
                                for field in model.fields:
                                    if (
                                        working_attributes[field.name] is None
                                        and field.not_nullable
                                    ):
                                        working_objects[model.name] = None

                                        if model.settings.get("critical"):
                                            raise IntegrityError(
                                                f"Critical model is invalid: Model: {model.name}, Field: {field.name} is null"
                                            )
                            ...
                            if model.name not in working_objects:
                                working_objects[model.name] = model.model(
                                    **working_attributes
                                )
                                ...
                                if model.settings.get("restriction") == "deferred":
                                    if type(working_objects[model.name].pk) is int:
                                        ImportSchemeRowDeferred(
                                            import_scheme=self,
                                            model=model.name,
                                            pkey_name=working_objects[
                                                model.name
                                            ]._meta.pk.name,
                ...
                ImportSchemeRowRejected(
                    import_scheme=self, errors=str(err), row=row
                ).save()
    ...
    def description_object(self) -> str:
        ...
        description: dict = {"files": {"secondary": {}}, "color_keys": {}, "models": []}
        ...
        for file_id, link in self.settings.get("file_links", {}).items():
            ...
            file_object["primary_field"] = primary_file.fields.get(
                pk=link["primary"]
            ).name
        ...
        for app in self.importer_object.apps:
            for model in app.models:
                model_object = {"name": model.fancy_name, "items": []}
                ...
                for item in self.items.filter(app=app.name, model=model.name):
                    ...
                    if item.strategy == "No Data":
                        ...
                        file_field: ImportSchemeFileField = (
                            ImportSchemeFileField.objects.get(pk=item.settings["key"])
                        )
                        ...
                        for field_key, field_value in item.settings.items():
                            file_field: ImportSchemeFileField = (
                                ImportSchemeFileField.objects.get(pk=field_value["key"])
                            )
...
class ImportSchemeFile(ImportBaseModel):
    ...
    name = models.CharField(max_length=255, null=False, blank=False)
    import_scheme = models.ForeignKey(
        ImportScheme, on_delete=models.CASCADE, related_name="files", editable=False
    )
    type = models.CharField(max_length=255)
    status = models.ForeignKey(
        ImportSchemeFileStatus,
        on_delete=models.DO_NOTHING,
        default=1,
        related_name="files",
    )
                ...
                return int(
                    subprocess.check_output(
                        [
                            "wc",
                            "-l",
                            settings.ML_IMPORT_WIZARD["Working_Files_Dir"]
                            + self.file_name,
                        ]
                    ).split()[0]
                )
    ...
    def rows(
        ...
        *,
        ...
        if self.base_type == "gff":
            for row in self._rows_from_gff_file(
    ...
    def find_row_by_key(
        ...
        *,
        ...
        if self.settings.get("has_db") and not connection:
            connection = self._get_db_connection()
        ...
        if type(key) is list:
            ...
            for item in key:
                row = self.find_row_by_key(
                    field=field, key=item, cache=cache, connection=connection
                )
                if row:
                    break
        ...
        if self.base_type == "text":
            if self.settings.get("has_db", False):
                row = connection.execute(
                    f"SELECT * FROM data WHERE \"{field}\"='{key}'"
                ).fetchone()
                return row
            else:
    ...
    def _rows_from_db(
        ...
        *,
        ...
        if limit_count:
            limit_bit = f" LIMIT {limit_count} "
        if specific_rows:
            where_bit = (
                f" WHERE rowid in ({','.join([str(row) for row in specific_rows])}) "
            )
        if offset_count:
            offset_bit = f" OFFSET {offset_count}"
    ...
    def _rows_from_text_file(
        ...
        *,
        ...
        with open(
            f"{settings.ML_IMPORT_WIZARD['Working_Files_Dir']}{self.file_name}", "r"
        ) as file:
            ...
            for row in reader:
                ...
                if (
                    not header_row
                    and specific_rows is not None
                    and read_count not in specific_rows
                ):
                    continue
    ...
    def _inspect_text_file(self, *, ignore_status: bool = False) -> None:
        ...
        if self.settings["first_row_header"]:
            if row_count > 26:
                specific_rows = [
                    int(row * (row_count - 1) / 25) for row in range(1, 26)
                ]
        else:
            if row_count > 25:
    ...
    def _inspect_gff_file(
        self, *, use_db: bool = False, ignore_status: bool = False
    ) -> None:
            ...
            for feature in islice(db.features_of_type(featuretype=feature_type), 5):
                ...
                for attribute in feature.attributes:
                    if attribute in attributes:
                        attributes[attribute] = attributes[attribute] | set(
    ...
    def _confirm_file_is_ready(
        self,
        *,
        ignore_status: bool = False,
        preinspected: bool = False,
        ...
        if self.type.lower() in ("gff", "gff3"):
            ...
            if NO_GFFUTILS:
                raise GFFUtilsNotInstalledError(
                    "gfutils is not installed: The file can't be inspected because GFFUtils is not installed. (pip install gffutils)"
                )

        if not ignore_status and self.status.uploaded == False:
            raise FileNotReadyError(
                f'File not marked as saved: {self} ({settings.ML_IMPORT_WIZARD["Working_Files_Dir"]}{self.file_name})'
            )
        ...
        if not os.path.exists(
            ...
            raise FileNotReadyError(
                f"File is missing from disk: {self} ({settings.ML_IMPORT_WIZARD['Working_Files_Dir']}{self.file_name})"
            )

        if not ignore_status and preinspected and self.status.preinspected == False:
            raise FileNotReadyError(
                f'File has not been preinspected: {self} ({settings.ML_IMPORT_WIZARD["Working_Files_Dir"]}{self.file_name})'
            )

        if not ignore_status and not inspected and self.status.inspected == True:
            raise FileNotReadyError(
                f'File already inspected: {self} ({settings.ML_IMPORT_WIZARD["Working_Files_Dir"]}{self.file_name})'
            )

        if not ignore_status and inspected and self.status.inspected == False:
            raise FileNotReadyError(
                f'File has not been inspected: {self} ({settings.ML_IMPORT_WIZARD["Working_Files_Dir"]}{self.file_name})'
            )
        ...
        if self.type.lower() in ("gff", "gff3"):
            if (
                ...
                raise FileNotReadyError(
                    f"DB file is missing from disk: {self} ({settings.ML_IMPORT_WIZARD['Working_Files_Dir']}{self.file_name}.db)"
                )
    ...
    def _create_db_from_text_file(self, *, replace_file: bool = False) -> None:
        ...
        if (
            ...
            if replace_file:
                ...
                raise FileExistsError(
                    f"SQLite3 DB already exists: {settings.ML_IMPORT_WIZARD['Working_Files_Dir']}{self.file_name}.db"
                )
        ...
        connection.executemany(
            "INSERT INTO columns VALUES(?)", [(column,) for column in columns]
        )
...
class ImportSchemeFileField(ImportBaseModel):
    ...
    import_scheme_file = models.ForeignKey(
        ImportSchemeFile,
        related_name="fields",
...
class ImportSchemeItem(ImportBaseModel):
    ...
    strategy = models.CharField(
        "Strategy for doing this import", max_length=255, null=True
    )
