...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            nargs="+",
            type=int,
            help="The ID(s) of the ImportFile(s) to inspect",
        )
    ...
    def handle(self, *args, **options):
        ...
        for import_file_id in options["import_file_id"]:
            ...
            if verbosity > 1:
                self.stdout.write(
                    f'Starting to inspect {import_scheme_file} ({settings.ML_IMPORT_WIZARD["Working_Files_Dir"]}{import_scheme_file.file_name}) file.'
                )

            try:
                import_scheme_file.inspect(
                    use_db=options["use_db"], ignore_status=options["ignore_status"]
                )
            ...
            if verbosity > 1:
                self.stdout.write(
                    f'You done inspected that {import_scheme_file} ({settings.ML_IMPORT_WIZARD["Working_Files_Dir"]}{import_scheme_file.file_name}) file!'
                )

            files_count += 1

        if not files_count:
            self.stdout.write(self.style.FAILURE(f"No files inspected."))
        elif files_count == 1:
            self.stdout.write(self.style.SUCCESS(f"1 file inspected."))
        else:
            self.stdout.write(self.style.SUCCESS(f"{files_count} files inspected."))
