...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default=None,
            type=int,
            help="Number of rows to import from the file.",
        )
        parser.add_argument(
            "--offset_count",
            nargs="?",
    ...
    def handle(self, *args, **options):
        ...
        for import_scheme_id in options["import_scheme_id"]:
            ...
            if verbosity > 1:
                self.stdout.write(
                    f"Starting to import {import_scheme} (import_scheme.importer)"
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"{import_scheme} ({import_scheme.id}) has been imported."
                )
            )
