...
class ModelTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.import_file_1 = ImportSchemeFile(
            name="test1.txt", import_scheme=cls.import_scheme
        )
    ...
    def test_import_scheme_hash_should_be_the_same_as_the_hash_of_the_raw_dict_from_settings(
        self,
    ):
        """import_scheme has should be the same as the hash from the raw dict from settings"""
        self.assertEqual(
            dict_hash(settings.ML_IMPORT_WIZARD["Importers"]["Genome"]),
            self.import_scheme.importer_hash,
        )
    ...
    def test_import_scheme_can_list_its_files(self):
        ...
        import_file_2 = ImportSchemeFile(
            name="test2.txt", import_scheme=self.import_scheme
        )
        import_file_2.save()

        self.assertEqual("test1.txt, test2.txt", self.import_scheme.list_files())
        self.assertEqual(
            "test1.txt<br>test2.txt", self.import_scheme.list_files(separator="<br>")
        )
    ...
    def test_import_scheme_should_be_able_to_add_item_if_its_not_there(self):
        ...
        self.import_scheme_item = self.import_scheme.create_or_update_item(
            app="app",
            model="model",
            field="field",
            strategy="raw_text",
            settings={"text": "this thing"},
        )
    ...
    def test_import_scheme_should_have_two_items_after_two_are_added(self):
        ...
        import_scheme_item_1 = self.import_scheme.create_or_update_item(
            app="app",
            model="model",
            field="field1",
            strategy="raw_text",
        ...
        import_scheme_item_2 = self.import_scheme.create_or_update_item(
            app="app",
            model="model",
            field="field1",
            strategy="file_field",
        ...
        import_scheme_item_3 = self.import_scheme.create_or_update_item(
            app="app",
            model="model",
            field="field2",
            strategy="stupid_test",
            settings={"file_field": 102},
        )
...
class LRUCacheThingsTests(TestCase):
    ...
    def test_cache_returns_value_stored_with_transaction_after_commit_and_rollback(
        self,
    ):
        """Cache should return None when given a key that doesn't exist"""
        self.assertEqual(self.cache.find(key="transaction"), "transaction test1")
...
class SimpleUtilsTest(TestCase):
    ...
    def test_split_by_caps_returns_a_one_item_list_when_given_a_string_with_no_caps(
        self,
    ):
    ...
    def test_fancy_name_returns_string_initial_caps_when_given_a_underscored_string(
        self,
    ):
    ...
    def test_fancy_name_returns_string_initial_caps_when_given_a_camelcased_string(
        self,
    ):
    ...
    def test_fancy_name_returns_string_with_capital_id_at_the_beginning_of_a_string(
        self,
    ):
    ...
    def test_deep_exists_returns_false_if_a_key_doesnt_exist_recursively(self):
        ...
        self.assertEqual(
            deep_exists(dictionary={"1": {"3": "test"}}, keys=["1", "7"]), False
        )
    ...
    def test_deep_exists_returns_true_if_a_key_exists_recursively(self):
        ...
        self.assertEqual(
            deep_exists(dictionary={"1": {"3": "test"}}, keys=["1", "3"]), True
        )

    def test_deep_exists_returns_true_if_a_key_exists_recursively_and_there_is_excess_dictionary(
        self,
    ):
        """deep_exists() should return True when a given key is not in the list, recursively, and there is excess dictionary"""
        self.assertEqual(
            deep_exists(dictionary={"1": {"3": {5: "test"}}}, keys=["1", "3"]), True
        )
...
class TemplateAndViewTests(SimpleTestCase):
    ...
    def test_root_should_have_genome_and_integrations_items_and_template_is_manager(
        self,
    ):
    ...
    def test_genome_post_should_result_in_new_ImportScheme_object_have_template_manager(
        self,
    ):
        """Use /import/Genome to add a genome import, test the return status,"""
        response = self.client.post(
            "/import/Genome",
            data={"name": "Test Importer from Page", "description": "testing"},
        )
