...
class ImporterModel(BaseImporter):
    ...
    def key_value_children(self) -> bool:
        ...
        return [
            field.key_value_linked_model
            for field in self.fields + self.unused_fields
            if field.is_key_value_link
        ]
...
class ImporterField(BaseImporter):
        ...
        **settings,
        ...
        for resolver in self.settings.get("resolvers", []):
            ...
            for argument in function.__code__.co_varnames[
                0 : function.__code__.co_kwonlyargcount
            ]:
                if argument.startswith("user_input_"):
                    ...
                    resolver_object["field_lookup_arguments"].append(
                        argument.replace("field_lookup_", "", 1)
                    )
    ...
    def is_key_value_link(self) -> bool:
        ...
        if self.field.related_model:
            ...
            if model_object := self.parent.parent.parent.apps_by_name.get(
                app, {}
            ).models_by_name.get(model, {}):
    ...
    def is_key_field(self) -> bool:
        ...
        if (
            not self.parent.is_key_value
            or self.parent.settings.get("key_field") != self.name
        ):
            return False
    ...
    def is_value_field(self) -> bool:
        ...
        if (
            not self.parent.is_key_value
            or self.parent.settings.get("value_field") != self.name
        ):
            return False
...
def setup_importers() -> None:
    ...
    for importer_setting, importer_value in settings.ML_IMPORT_WIZARD[
        "Importers"
    ].items():
        ...
        for app in importer_value.get("apps", []):
            working_app: ImporterApp = ImporterApp(
                parent=working_importer, name=app.get("name", "")
            )
            ...
            if app.get("include_models", []):
                models = [
                    ...
                    in [
                        app_model.__name__
                        for app_model in apps.get_app_config(app["name"]).get_models()
                    ]
                ]
            ...
            if not models:
                models = filter(
                    lambda model: model.__name__ not in app.get("exclude_models", []),
            ...
            for model_object in models:
                ...
                if deep_exists(dictionary=app, keys=["models", model_object.__name__]):
                    model_settings: dict = {
                        setting: value
                        for setting, value in model.items()
                        if setting not in exclude_keys
                    }

                working_model: ImporterModel = ImporterModel(
                    parent=working_app,
                ...
                for field in [
                    ...
                    if field.name not in model.get("exclude_fields", [])
                ]:
                    ...
                    field_settings: dict = {
                        setting: value
                        for setting, value in model.get("fields", {})
                        .get(field.name, {})
                        .items()
                        if setting not in exclude_keys
                    }

                    working_field: ImporterField = ImporterField(
                        parent=working_model,
                        name=field.name,
                        field=field,
                        **field_settings,
                    )
            ...
            while len(working_app.models) > len(working_app.models_by_import_order):
                ...
                for model_object in working_app.models:
                    ...
                    for field in foreign_keys:
                        if field.field.related_model.__name__ in [
                            model.name for model in working_app.models
                        ] and field.field.related_model.__name__ not in [
                            model.name for model in working_app.models_by_import_order
                        ]:
                ...
                if model_by_import_count >= len(working_app.models_by_import_order):
                    log.debug(
                        [object.name for object in working_app.models_by_import_order]
                    )
                    raise UnresolvedInspectionOrder(
                        "Order of importing models can't be resolved.  Potential circular ForeignKeys?"
                    )
